...
export default defineRailway(() => {
  ...
  const bizstackPerks = service("bizstack-perks", {
    ...
    env: { AUTO_INGEST_OVERRIDE: preserve(), BIZSTACK_ADMIN_PASS: preserve(), BIZSTACK_ADMIN_USER: preserve(), BOT_API_TOKEN: preserve(), COMMERCIAL_LENDER_ENDPOINT: preserve(), DOMAIN: preserve(), FINNHUB_DATA_KEY: preserve(), Key: preserve(), PRICE_ID: preserve(), PUBLIC_BASE_URL: preserve(), STRIPE_SECRET_KEY: preserve(), STRIPE_WEBHOOK_SECRET: preserve(), SUPPORT_EMAIL: preserve(), TA_TRACKER_ID: preserve(), TWILIO_ACCOUNT_SID: preserve(), TWILIO_AUTH_TOKEN: preserve(), TWILIO_NUMBER: preserve(), Value: preserve() },
  });
